...
test ('convert text with <style/>', async (t) => {
  const expected = fs.readFileSync(path.resolve('test/fixtures/scrapbox/style.txt')).toString()
  const compiler = new Compiler(fs.readFileSync(path.resolve('test/fixtures/html/style.html')))
  const input = await compiler.compile().result
  t.is(expected, input)
})
